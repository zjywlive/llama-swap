  ...
  <div class="p-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2 items-center">
        <button class="btn border-0" onclick={toggleFontSize} title={$tx.logs.changeFontSize}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
            <path d="M2 4v3h5v12h3V7h5V4H2zm19 5h-9v3h3v7h3v-7h3V9z"/>
          </svg>
        </button>
        <button class="btn border-0" onclick={toggleWrapText} title={$tx.logs.toggleTextWrap}>
          {#if $wrapTextStore}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
        ...
        <button class="btn border-0" onclick={toggleFilter} title={$tx.logs.toggleFilter}>
          {#if $showFilterStore}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
    ...
    {#if $showFilterStore}
      <div class="mt-2 flex gap-2 items-center w-full">
        <input
          ...
          placeholder={$tx.logs.filterPlaceholder}
          bind:value={filterRegex}
        />
        <button class="pl-2" onclick={() => (filterRegex = "")} aria-label={$tx.logs.clearFilter}>
